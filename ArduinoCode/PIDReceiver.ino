...
void loop(){
  ...
  {
    yawStick = map(newJoyStk->xL, 470,0,0,-150); //maximum 20% might change for no massive pwm
  }

  else if(newJoyStk->xL >=550) //Left yaw
  {
    yawStick = map(newJoyStk->xL, 550,1023,0,+150);
  }
...
//
  ...
  PIDInputYaw = PIDcontroller(Pyaw, Iyaw, Dyaw, ErrorYaw, PrevErrorYaw, PrevItermYaw,150);
  PrevErrorYaw = ErrorYaw;
  PrevItermYaw = PIDReturnIterm;


  PIDInputPitch = PIDcontroller(Ppitch, Ipitch, Dpitch, ErrorPitchAngle, PrevErrorPitchAngle, PrevItermPitchAngle, 50);
  PrevErrorPitchAngle = ErrorPitchAngle;
  PrevItermPitchAngle = PIDReturnIterm;


  leftMotor = throttleLeftStick - PIDInputYaw;
  rightMotor = throttleLeftStick + PIDInputYaw;
  frontMotor = throttleRightStick - PIDInputPitch;
  backMotor = throttleRightStick + PIDInputPitch;
  ...
  ledcWrite(BACK_MOTOR_PIN, backMotor);
